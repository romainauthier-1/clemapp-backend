...
router.get("/all/:isDemo", async (req, res) => {
	const isDemo = req.params.isDemo;
	console.log(isDemo);
	try {
		if (isDemo === "true") {
			// user en démo

			const demoSurprises = await Surprise.find({ demo: true }).populate(
				"organizedBy",
				"name",
			);

			if (demoSurprises.length > 0) {
				res.status(302).json({ result: true, allSurprises: demoSurprises });
			} else {
				res
					.status(404)
					.json({ result: false, message: "[DEMO] Aucune surprise trouvée." });
			}
		} else {
			// user réel

			const realSurprises = await Surprise.find({ demo: false }).populate(
				"organizedBy",
				"name",
			);

			if (realSurprises.length > 0) {
				res.status(302).json({ result: true, allSurprises: realSurprises });
			} else {
				res
					.status(404)
